...
//
// DESCRIPTION:
//		This machine initializes all the nodes and runs the state
// machine 		that synchronizes the moves among the nodes; the
// synchronization 		is simply waiting for all nodes to send their
// moves to their nodes, 		triggering them all at the same time,
// then waiting for them all to 		finish before issuing any more
// moves
//
//																			   *
//******************************************************************************
...
Supervisor::Supervisor(vector<Axis *> theAxes, SysManager &myMgr)
  ...
  for (Uint16 iAxis = 0; iAxis < m_axes.size(); iAxis++) {
    ...
    m_axisSentCommand.push_back(false);
...
//
void Supervisor::WaitForKick() { WaitForCondition(m_nodesKicked); }
...
//		Nodes will wait to be kicked
//
void Supervisor::WaitForDone() { WaitForCondition(m_nodesDone); }

void Supervisor::WaitForIdle() { WaitForCondition(m_nodesIdle); }

bool Supervisor::ReturnDoneCondition() { return ReturnCondition(m_nodesDone); }
...
//		Nodes will wait to be kicked
//
void Supervisor::SignalReadyCommand() { SetCondition(m_commandsReady); }
...
//
void Supervisor::SignalCommandSent(Uint16 nodeNum) {
  SignalPerNode(m_axisSentCommand, m_nodesMoved, nodeNum);
}
//																			   *
//******************************************************************************
...
//
void Supervisor::SignalCommandDone(Uint16 nodeNum) {
...
//
//	DESCRIPTION:
//		The Supervisor's state machine thread; tells each Axis to create
// its 		own thread and initialize itself, then runs the state machine to
...
void Supervisor::SuperMain() {
  ...
  ResetCondition(m_axisSentCommand, m_nodesMoved);
  ResetCondition(m_axisDone, m_nodesDone);
  ResetCondition(m_commandsReady);
  ...
  while (!m_quitting) {
    try {
      ...
      case SUPER_WAIT_FOR_IDLE:
        ...
        ResetCondition(m_axisIdle, m_nodesIdle);
        {
          std::unique_lock<std::mutex> lock_m(*mutex1, defer_lock);
          lock_m.lock();
          super_idle = true;
          command_done = false;
        ...
        // lk.try_lock_for(std::chrono::milliseconds(100));
        {
          ...
          /*while (!command_ready) {
            cout << "hi" << endl;
            main_cond->wait(lock_m);
          }*/

          // cout << "waiting for command ready: " << command_ready << endl;
          main_cond->wait(lock_m, [&] { return command_ready; });
        }
        if (command_type == Home) {
        } else if (command_type == Move) {
          SetCondition(m_nodesKicked);
          m_state = SUPER_WAIT_FOR_COMMAND_SENT;
        } else {
        }
        ...
        break;
      case SUPER_WAIT_FOR_COMMAND_SENT:
        ...
        ResetCondition(m_nodesKicked);
        ResetCondition(m_axisSentCommand, m_nodesMoved);
        m_axes.at(0)->MyNode()->Port.Adv.TriggerMovesInGroup(2);
        m_state = SUPER_WAIT_FOR_COMMAND_DONE;
        break;
      case SUPER_WAIT_FOR_COMMAND_DONE:
