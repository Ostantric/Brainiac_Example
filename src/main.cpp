...
#define FILENAME "gcode/H_Logo_06mm.gcode"
 ...
 *					This handler simply prints out the attention
 *information to the console window. Parameters: Input:	detected
 *- contains the attention information Return:		none
 *****************************************************************************/
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (size_t iPort = 0; iPort < portCount; iPort++) {
      ...
      for (unsigned iNode = 0; iNode < myPort.NodeCount(); iNode++) {
        ...
        if (theNode.Info.NodeType() != IInfo::CLEARPATH_SC_ADV) {
          printf("---> ERROR: Uh-oh! Node %d is not a ClearPath-SC Advanced "
                 "Motor\n",
                 iNode);
          nodeTypesGood = false;
        }
        ...
        if (nodeTypesGood) {
          ...
          if (!theNode.Setup.AccessLevelIsFull()) {
            printf(
                "---> ERROR: Oh snap! Access level is not good for node %u\n",
                iNode);
            accessLvlsGood = false;
          } else {
            // Set the move distance based on where it is in the network
            // listOfAxes.at(iNode)->SetMove(1,2);
    ...
    // If we have full access to the nodes and they are all ClearPath-SC
    // advanced nodes, then continue with the example
      int i = 0;
    ...
    if (nodeTypesGood && accessLvlsGood) {
      ...
      if (file.is_open()) {
        ...
        while (getline(file, line)) {
          if (line.at(0) == 'G' && line.at(1) == '1') {
            printf("[%i] %s\n", i, line.c_str());
            listOfAxes.at(0)->SetMove(10,250);
            i++;
          } else {
          }
          if (i == 15) {
            msgUser("To see the next 16 lines press ENTER.");
            ready_flag=false;
            i = 0;
          }
        }
      ...
      msgUser(
          "Press any key to stop the machine."); // pause so the user can see
