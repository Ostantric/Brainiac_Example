...
private:
	...
	bool m_nodesIdle, m_nodesMoved, m_nodesDone, m_nodesKicked, m_nodesReady, m_commandsReady;
	// Flags to keep track of which nodes have reported back
	// for various conditions
	vector<bool> m_axisReady;
	vector<bool> m_axisIdle;
	vector<bool> m_axisSentCommand;
	...
	enum SuperStateEnum {
		SUPER_IDLE,
		SUPER_WAIT_FOR_IDLE,
		SUPER_WAIT_FOR_COMMAND_SENT,
		SUPER_WAIT_FOR_COMMAND_DONE,
		SUPER_EXITED
	};
...
public:
	...
	bool command_done,super_idle,command_ready;
	enum CommandType {
		Idle,
		Home,
		Move
	...
	void SignalCommandSent(Uint16 nodeNum);	// Nodes tell the supervisor when they've sent their move
	void SignalCommandDone(Uint16 nodeNum);	// Nodes tell the supervisor whne their move is done
